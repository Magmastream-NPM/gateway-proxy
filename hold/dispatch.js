		...
		if (parsedEvent) {
			switch (parsedEvent.type) {
				...
				case "InvalidateSession":
					console.debug(`[Shard ${shardId}] Session invalidated, resumable: ${parsedEvent.canResume}`);
...
function updateShardStatistics(shardIdStr, shardState, connectionState, latencies) {
	const connectionStatus = getConnectionStatus(connectionState);

	const latency = latencies.length ? latencies[0] : NaN;

	metrics.histogram("gateway_shard_latency_histogram", { shard: shardIdStr }).record(latency);
	metrics.gauge("gateway_shard_latency", { shard: shardIdStr }).set(latency);
	metrics.histogram("gateway_shard_status", { shard: shardIdStr }).record(connectionStatus);

	const stats = shardState.guilds.stats();

	metrics.gauge("gateway_cache_emojis", { shard: shardIdStr }).set(stats.emojis());
	metrics.gauge("gateway_cache_guilds", { shard: shardIdStr }).set(stats.guilds());
	metrics.gauge("gateway_cache_members", { shard: shardIdStr }).set(stats.members());
	metrics.gauge("gateway_cache_presences", { shard: shardIdStr }).set(stats.presences());
	metrics.gauge("gateway_cache_channels", { shard: shardIdStr }).set(stats.channels());
	metrics.gauge("gateway_cache_roles", { shard: shardIdStr }).set(stats.roles());
	metrics.gauge("gateway_cache_unavailable_guilds", { shard: shardIdStr }).set(stats.unavailable_guilds());
	metrics.gauge("gateway_cache_users", { shard: shardIdStr }).set(stats.users());
	metrics.gauge("gateway_cache_voice_states", { shard: shardIdStr }).set(stats.voice_states());
}
