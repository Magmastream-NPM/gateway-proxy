...
async function handleClient(ws, state, useZlib) {
	...
	ws.on("message", async (msg) => {
		...
		switch (payload.op) {
			...
			case 2: // Identify
				...
				shardForwardTask = forwardShard(identify, shardSender, streamWriter, ws);
				break;
			...
			case 6: // Resume
				...
				if (shardSender) {
					shardForwardTask = forwardShard(resume, shardSender, streamWriter, ws);
					ws.send(RESUMED);
				} else {
					ws.send(INVALID_SESSION);
				}
	...
	await sinkFromQueue(ws._socket.remoteAddress, useZlib, compressRx, messageStream, ws);
}
